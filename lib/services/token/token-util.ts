...
export function tokenGetAmountForAddress(
  ...
  return (
    tokenAmounts.find((amount) => amount.address === address.toLowerCase())?.amount || defaultValue
  );
}
...
export function tokenFindTokenAmountForAddress(
  ...
  return (
    tokenAmounts.find((amount) => amount.address === address) || { address, amount: defaultValue }
  );
}

export function tokenAmountsConcatenatedString(
  tokenAmounts: TokenAmountHumanReadable[],
  tokens: TokenBase[],
): string {
  return tokenAmounts
...
export function tokenAmountsGetArrayFromMap(
  amountMap: AmountHumanReadableMap,
): TokenAmountHumanReadable[] {
  return map(
...
export function tokenFormatAmountPrecise(
  amount: AmountHumanReadable | number,
  precision: number = 12,
) {
  const amountNum = typeof amount === 'string' ? parseFloat(amount) : amount;
